...
var storeLocationDataUrl = 'data/Airport-Tab.txt';
        ...
        .then(function (text) {
            ...
            for (var i = 1; i < lines.length; i++) {
                ...
                if (row.length >= numColumns) {
                    ...
                    features.push(new atlas.data.Feature(new atlas.data.Point([parseFloat(row[header['longitude_deg']]), parseFloat(row[header['latitude_deg']])]), {
                        id: row[header['id']],
                        ident: row[header['ident']],
                        type: row[header['type']],
                        name: row[header['name']],
                        elevation_ft: row[header['elevation_ft']],
                        continent: row[header['continent']],
                        country: row[header['iso_country']],
                        region: row[header['iso_region']],
                        municipality: row[header['municipality']],
                        service: row[header['scheduled_service']],
                        gps_code: row[header['gps_code']],
                        iata_code: row[header['iata_code']],
                        }));
                }
            }
...
//Creates an addressLine2 string consisting of Name, Municipality, Region and IATA Code.
function getAddressLine2(properties) {
    ...
    if (properties['iso_region']) {
        html.push(', ', properties['iso_regiom']);
    }

    if (properties['iata_code']) {
        html.push(' ', properties['iata_code']);
    }
